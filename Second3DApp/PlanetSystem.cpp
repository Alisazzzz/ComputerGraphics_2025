...
void PlanetSystem::Initialize()
{
	...
	Vector4 color1Planet = Vector4(0.05f, 0.8f, 0.1f, 1.0f);
	Vector4 color2Planet = Vector4(0.0f, 0.1f, 0.0f, 1.0f);

	Vector4 color1Lune = Vector4(0.5f, 0.5f, 0.5f, 1.0f);
	Vector4 color2Lune = Vector4(0.7f, 0.7f, 0.7f, 1.0f);

	for (int i = 0; i < 100; i++) {
		float radius = 0.01 * (rand() % 101);
		float spinSpeedRand = 0.01 * (rand() % 101);
		float orbitalSpeedRand = 0.01 * (rand() % 101);

		Vector4 color1 = color1Planet;
		Vector4 color2 = color2Planet;
		float currentOrbit = 1.0f;

		PlanetObject* parent;
		int isLune = rand() % 101;
		if ((isLune > 40) || (objects.size() == 0)) {
			parent = center;
			currentOrbit = orbit;
			orbit += 2.0f;
			radius += 0.5f;
			...
			int parentIndex = rand() % (objects.size());
			parent = objects.at(parentIndex);
			color1 = color1Lune;
			color2 = color2Lune;
			if (radius > 0.6f) {
				radius -= 0.5f;
		...
		TriangleComponent* planetMesh = new TriangleComponent(game);
		Mesh mesh = MeshGenerator::getInstance()->getSphere(radius, 24, 24, color1, color2);
		planetMesh->Initialize(L"./Shaders/MyVeryFirstShader.hlsl", mesh.points, mesh.indeces, strides, offsets, false);
		game->components.push_back(planetMesh);
		...
		PlanetObject* planet = new PlanetObject{
			...
			0.0f,
			orbitalSpeedRand,
			Vector3(currentOrbit, 0.0f, 0.0f),
			Vector3(currentOrbit, 0.0f, 0.0f),
			parent
		};
		...
		if ((isLune > 40) || (objects.size() == 0)) {
			OrbitCamera* cam = new OrbitCamera(game);
			cam->Initialize(camOrbit, planet->position - camOrbit, planet->position);
			OrbitCameraObject* camObject = new OrbitCameraObject{ cam, planet };
			cameras.push_back(camObject);
			game->components.push_back(cam);
		}
	}
